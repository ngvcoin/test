...
cd $HOME
if [ ! $VMHOST ]; then
	read -p "Введите IP сервера VictoriaMetrics: " VMHOST
fi
if [ ! $OWNER ]; then
...
if [ ! $HOSTNAME1 ]; then
	read -p "Введите название своего сервера: " HOSTNAME1
fi
echo 'Название вашего сервера: ' $HOSTNAME1
sleep 1
echo 'export HOSTNAME1='$HOSTNAME1 >> $HOME/.profile
